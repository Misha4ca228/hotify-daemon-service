...
from daemon.services.create_character import get_user_character
            ...
            async for message in queue_iter:
                async with message.process():
                    ...
                    async with sessionmaker() as session:
                        ...
                        character_name = None
                        user_character_face = None
                        if dialog.character_id.startswith("user_"):
                            char_id = int(dialog.character_id.split("_")[-1])
                            character = await get_user_character(session, char_id)
                            user_character_face = character.face_url.split("/")[-1]
                        else:
                            character = await get_character_by_id(session, int(dialog.character_id))
                        ...
                        if task.image_args:
                            ...
                            url = await asyncio.to_thread(save_and_gen, node.worker_url, user_character_face,
                                                          character_name, **function_args)
